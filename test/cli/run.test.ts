...
describe("runCommand", () => {
  it("returns exit code 0 for passing suites", async () => {
    const code = await runCommand({
      ...
      config: { suites: [], reporter: "json", verbose: false, threshold: undefined, output: undefined, failOnError: false },
    });
    expect(code).toBe(0);
  });
  ...
  it("returns exit code 1 when below threshold", async () => {
    const code = await runCommand({
      ...
      config: { suites: [], reporter: "json", verbose: false, threshold: 0.8, output: undefined, failOnError: false },
    });
    expect(code).toBe(1);
  });
  ...
  it("returns exit code 2 when errors and --fail-on-error", async () => {
    const code = await runCommand({
      ...
      config: { suites: [], reporter: "json", verbose: false, threshold: undefined, output: undefined, failOnError: true },
    });
    expect(code).toBe(2);
  });
